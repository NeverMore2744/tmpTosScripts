        ...
        if (type == 'W') { // write request
          for (uint64_t i = 0; i < length; i += 1) {
            if (indexMap_->get(offset + i) != 0) {
              uint32_t currDis = getDistance(offset + i);

              if (lastDistanceMap_->get(offset + i) != 0) {
                uint32_t lastDis = lastDistanceMap_->get(offset + i) - 1;
                intervalHistogramByDataAmount_->inc(lastDis * max_dis + currDis);
              }
              lastDistanceMap_->put(offset + i, currDis + 1);
            }
